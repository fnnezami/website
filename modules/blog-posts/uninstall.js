module.exports = async function uninstall(ctx) {
  ...
  const moduleDir = (ctx && ctx.moduleDir) || path.join(process.cwd(), "modules", "blog-posts");
  ...
  try {
    ...
    // drop blog_posts table and any related objects (idempotent)
    await pg.query(`DROP TABLE IF EXISTS public.blog_posts;`);
  ...
  return { ok: true, dropped: ["blog_posts"] };
};
