...
export async function GET(req: Request) {
  try {
    ...
    for (const r of data || []) {
      ...
      if (bucketParam === "hour") {
        // UTC hour bucket
        key = new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate(), d.getUTCHours(), 0, 0, 0)).toISOString();
      } else {
        // UTC day bucket
        key = new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate(), 0, 0, 0, 0)).toISOString();
      }
